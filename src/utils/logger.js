 ...
 * Uses messenger.storage.local for cross-context synchronization
 */
...
class DebugLogger {
    ...
    async init() {
        try {
            const result = await messenger.storage.local.get('debugMode');
    ...
    listenForChanges() {
        if (typeof messenger !== 'undefined' && messenger.storage && messenger.storage.onChanged) {
    ...
    async enable() {
        this.enabled = true;
        if (typeof messenger === 'undefined' || !messenger.storage) return;
        try {
            await messenger.storage.local.set({ debugMode: true });
        } catch (e) {}
    }
    ...
    async disable() {
        ...
        if (typeof messenger === 'undefined' || !messenger.storage) return;
        try {
            await messenger.storage.local.set({ debugMode: false });
        } catch (e) {}
    }
...
window.AutoSortPlus.logger = logger;
